...
def check_prediction_intervals(
    ...
    for model_name, _ in hyperparameters.items():
        ...
        df = pd.DataFrame(
            ...
            "pred_len": upper_bounds.values - lower_bounds.values
        }        
        )
...
def plot_results(
    ...
    df = results.groupby(["model_name", "h"], as_index=False).agg(
        ...
        mean_squared_error=("squared_error", "mean"),   # Mean squared error for each h
        RMSE=("squared_error", lambda x: np.sqrt(x.mean())),
        avg_pred_length = ("pred_len", "mean")
    )
    ...
    p_coverage = (
        ...
        labs(title = plot_title, x = "Horizon(h)", y = "coverage_accuracy") +
    ...
    p_armse = (
        ggplot(df_armse, aes(x="Horizon", y="Average RMSE", group="model_name")) + 
        geom_line() + 
        geom_point() + 
        facet_wrap("~model_name", scales="fixed") +
    ...
    return [p_coverage, p_error, p_pred_len, p_armse]
