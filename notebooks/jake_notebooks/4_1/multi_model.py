...
def run_experiment(
    ...
    train_full_df = "./ma_output/train_exp.parquet"
    train_full_df = pd.read_parquet(train_full_df)
    train_full_df = train_full_df[train_full_df["theta"] == theta]
    
    test_full_df = "./ma_output/test_exp.parquet"
    test_full_df = pd.read_parquet(test_full_df)
    test_full_df = test_full_df[test_full_df["theta"] == theta]
    
    patch_hyperparam_df = "./data/PatchTST_hyperparams.parquet"
    patch_hyperparam_df = pd.read_parquet(patch_hyperparam_df)
...
def make_dir_name(
    ...
    dir_name = f"{num_runs}_{theta}_{train_size}_{prediction_length}_{eval_metric}_{ci_level}_{time_limit}_{max_epochs}"
    return dir_name
...
def plot_results(
    ...
    plot_title = make_dir_name(num_runs, theta, train_size, prediction_length, eval_metric, ci_level, time_limit, max_epochs)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--theta", type=float, required=True, help="Theta of the MA model, e.g., 0.5")
    ...
    results = run_experiment(
        num_runs, theta, train_size, prediction_length, eval_metric, ci_level, time_limit, verbosity, max_workers
    )
    results_plot = plot_results(results, num_runs, theta, train_size, prediction_length, eval_metric, ci_level, time_limit, max_epochs)

    ################################################################################
    # Save the results
    ################################################################################

    dir_name = make_dir_name(num_runs, theta, train_size, prediction_length, eval_metric, ci_level, time_limit, max_epochs)
